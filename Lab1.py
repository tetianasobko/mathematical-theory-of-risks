...
def draw_axes(y_top):
    ...
    canvas.create_line(50, 0, 50, CANVAS_H - dy)
    canvas.create_line(CANVAS_W - 50, 0, CANVAS_W - 50, CANVAS_H - dy)

    # поділки осі Х
    canvas.create_text(50, cy + 12, text=str(0), font="Verdana 9")
    canvas.create_text(CANVAS_W - 50, cy + 12, text=str(1), font="Verdana 9")
    ...
    while y > -1:
        if y == 0:
            ...
            continue
        y_canvas = (y - y_top) * dy
        canvas.create_line(47, -y_canvas, 53, -y_canvas)
        canvas.create_text(25, -y_canvas, text=str(y), font="Verdana 9")
        y -= 1
    ...
    while y > -1:
        if y == 0:
            ...
            continue
        y_canvas = (y - y_top) * dy
        canvas.create_line(CANVAS_W - 53, -y_canvas, CANVAS_W - 47, -y_canvas)
        canvas.create_text(CANVAS_W - 25, -y_canvas, text=str(y), font="Verdana 9")
        y -= 1
    return dx, dy
...
def draw_line(y_start, y_end, max_val, color):
    ...
    canvas.create_line(50, (max_val - y_start) * dy, CANVAS_W - 50, (max_val - y_end) * dy, fill=color, width = 2)
...
result_label.grid(row=len(labels) + 3, columnspan=4, padx=5, pady=5)
